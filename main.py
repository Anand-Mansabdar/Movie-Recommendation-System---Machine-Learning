...
app = FastAPI(title="Movie Recommendation", version="1.0")
...
class TFIDFRecommendedItem(BaseModel):
    title: str
    score: float
...
class SearchBundleResponse(BaseModel):
    ...
    tfidf_recommendations: List[TFIDFRecommendedItem]
...
def build_title_to_idx_map(indices: Any) -> Dict[str, int]:
    ...
    try:
        ...
        raise RuntimeError(
            "indices.pkl must be dict or pandas Series" 
        )

def get_local_index_by_title(title: str) -> int:
    global TITLE_TO_IDX
    if TITLE_TO_IDX is None:
        raise HTTPException(status_code=500, detail="TF-IDF index map not standardized")
    key = _norm_title(title)
    if key in TITLE_TO_IDX:
        return int(TITLE_TO_IDX[key])
    raise HTTPException(
        status_code=404, detail=f"Title not found: {title}"
    )
...
def recommend_titles(
    ...
    idx = get_local_index_by_title(query_title)
